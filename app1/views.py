...
def groupsummary(request,lk):#ann
    ...
    #  print(list1[1])
     ...
     for p in  list1:
      ...
      return render(request,'groupsummary.html',{'msg1':msg1,'subgrp':subg1,'total1':total1,'s':s,'d':d})
    if n==2:
    ...
    #   for t in vals: 
    #    n=vals[0]
    #   cd=Ledger_vouchers_new.objects.filter(Group_id=2) 
    #   print(cd)
    #   c=sum(cd.values_list('credit',flat=True))
    #   print(c)  
    #   d=sum(cd.values_list('debit',flat=True))
    ...
    #   else :
    #         s=d-c
    #   v=Ledger_vouchers_new.objects.filter(Group_id=2)
    # try:
    #     v=Ledger_vouchers_new.objects.filter(Group=2)
    # except Ledger_vouchers_new.DoesNotExist:
    ...
    return render(request,'groupsummarydb.html',{'msg1':msg1,'ledg':ledg,'total1':total1,'s':s}) 
...
def investments(request):
    ...
    #         return redirect('/')
        ...
        #   ledg=Ledger_vouchers_new.objects.raw('SELECT app1_tally_ledger.id,app1_tally_ledger.name,sum(app1_ledger_vouchers_new.debit) as debit,sum(app1_ledger_vouchers_new.credit) as credit FROM `app1_ledger_vouchers_new` inner join app1_tally_ledger on app1_ledger_vouchers_new.ledger_id=app1_tally_ledger.id group by app1_ledger_vouchers_new.ledger_id')
        #   return render(request,'groupsummaryinvestments.html',{'cmp':tally,'led':talled,'deb_balance':deb_balance})
        
        # except tally_ledger.DoesNotExist:
        # # Handle the case where the object doesn't exist
        
        #  return render(request,'groupsummaryinvestments1.html')
...
# payment_voucher_editpage
def Daybook_edit(request, voucher_id):
    voucher = get_object_or_404(payment_voucher, id=voucher_id)
    payment_particulars = voucher.payment_particulars_set.all()
    return render(request, 'daybook_edit.html', {'voucher': voucher, 'payment_particulars': payment_particulars})
...
def Daybook_edit_1(request,voucher_id):
    ...
    return render(request,"daybook_edit_1.html",{'voucher' : voucher, 'receipt_particulars': receipt_particulars})
